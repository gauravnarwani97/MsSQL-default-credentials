...
def connect_mssql_ip_port(ip, port, user="", passwd=""):
  """
...
def connect_db(ip, port, db):
   ...
   Function to connect MsSQL db with default cred
...
def main(ip, port):
    ...
    success = []
    if port == 1433:
        connect_db(ip, port, connect_mssql_ip_port)
		
if __name__ == "__main__":
    if len(sys.argv) != 3:
      sys.exit("Usage: python MsSQL_default_creds.py ip port")
